...
const RippleGrid: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    // WebGL BUG FIX 1: Set clear color explicitly to transparent to avoid the deafult solid white background.
...
void main() {
    // WebGL BUG FIX 2: Safeguard against Zero division.
    float aspectRatio = (iResolution.y > 0.0) ? (iResolution.x / iResolution.y) : 1.0;
    vec2 uv = vUv * 2.0 - 1.0;
    uv.x *= aspectRatio;
    ...
    // WebGL BUG FIX 3: Ensure resolution doesn't hang at 0 due to timing with ResizeObserver
    let rafId: number;
    ...
    const render = (t: number) => {
      ...
      // Keep track of the request so we can cancel it on unmount
    ...
    return () => {
      // WebGL BUG FIX 4: Cancel animation frame so WebGL contexts don't crash when HMR reloading
